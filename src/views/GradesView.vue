...
const { grades, totalItems, isSuccess, totalPages, currentPage, isLoading, fetchGrades } = useGradesStore();
...
<div class="title-container">
  ...
  <!-- Paginación -->
  <div style="margin-bottom: 5px;">Total grados: {{ totalItems }}</div>

  <div class="pagination">
      <span @click="fetchGrades(currentPage - 1)" class="arrow" :class="{ 'disabled': currentPage === 0 }">
        <i class="fa-solid fa-chevron-left"></i>
      </span>

      <template v-for="page in totalPages" :key="page">
        <span @click="fetchGrades(page - 1)" class="pagination-number" :class="{ 'active': currentPage + 1 === page }">{{ page }}</span>
      </template>
      <span @click="fetchGrades(currentPage + 1)" class="arrow" :class="{ 'disabled': currentPage >= totalPages - 1 }">
        <i class="fa-solid fa-chevron-right"></i></span>
    </div>


    <Transition>
      <GradesList v-if="!isLoading" :grades="grades"/>
    </Transition>
